...
// #define STATUS_SHOWCLIENTTYPE         128 // "h" (human), "b" (bot)
// svc_status 247 = all except for STATUS_SPECTATORS_AS_PLAYERS
pub(crate) const COMMAND: &[u8] = b"\xff\xff\xff\xffstatus 247";
...
impl TryFrom<&[u8]> for StatusResponse {
    type Error = Error;
...
mod tests {
    ...
    fn test_status119response_parse() -> Result<()> {
        ...
        {
            let res = StatusResponse::try_from([0].as_slice());
            assert_eq!(
        ...
        {
            let res = StatusResponse::try_from([255, 255, 255, 255, 110, 0].as_slice());
            assert_eq!(
                res.unwrap_err().to_string(),
        ...
        // with clients
        {
            ...
            let res = StatusResponse::try_from(bytes)?;
